...
class CtrlCarrito extends Controlador {
    public function agregar() {
        ...
        if (is_object($_SESSION['carrito'])){
            $_SESSION['carrito']->agregar($_GET['idCarrito'],1);
            
            if(isset($_GET['url'])){
                switch($_GET['url']){
                    case 'detalles':
                        header("Location: ?ctrl=CtrlProducto&accion=verDetalles&idProducto=".$_GET['idCarrito']);
                        exit();
                        break;
                    case 'carrito':
                        $this->mostrar();
                        ...
                        break;
                    default:
                        header("Location: ?ctrl=CtrlProducto&accion=getCatalogo&idProducto=".$_GET['idCarrito']);
                        exit();
                }
            }else{
                header("Location: ?ctrl=CtrlProducto&accion=getCatalogo&idProducto=".$_GET['idCarrito']);
    ...
    /*public function agregarMesa() {
        ...
        if (is_object($_SESSION['carrito'])){
            $_SESSION['carrito']->agregar($_GET['idMesa'],1);
            
            if(isset($_GET['url'])){
                switch($_GET['url']){
                    case 'detalles':
                        header("Location: ?ctrl=CtrlMesa&accion=verDetallesMesa&idMesa=".$_GET['idCarrito']);
                        exit();
                        break;
                    case 'carrito':
                        $this->mostrar();
                        ...
                        break;
                    default:
                        header("Location: ?ctrl=CtrlMesa&accion=getCatalogoMesa&idMesa=".$_GET['idCarrito']);
                        exit();
                }
            }else{
                header("Location: ?ctrl=CtrlMesa&accion=getCatalogoMesa&idMesa=".$_GET['idCarrito']);
    ...
    public function sacar() {
        if (isset($_SESSION['carrito'])){
            ...
            $_SESSION['carrito']->sacar($_GET['idCarrito'],$cant);
        }
